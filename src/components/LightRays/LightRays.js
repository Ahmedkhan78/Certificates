  ...
  useEffect(() => {
    ...
    const initializeWebGL = async () => {
      ...
      const updatePlacement = () => {
        ...
        const MIN_WIDTH = 300,
          MIN_HEIGHT = 300;
        const w = Math.max(wCSS * dpr, MIN_WIDTH);
        const h = Math.max(hCSS * dpr, MIN_HEIGHT);
        ...
        const scaleFactor = wCSS < 600 ? 0.7 : 1;
        uniforms.rayLength.value = rayLength * scaleFactor;
        uniforms.lightSpread.value = lightSpread * scaleFactor;
      };
      ...
      const loop = (t) => {
        ...
        if (followMouse && mouseInfluence > 0) {
          ...
          smoothMouseRef.current.x =
            smoothMouseRef.current.x * smooth +
            mouseRef.current.x * (1 - smooth);
          smoothMouseRef.current.y =
            smoothMouseRef.current.y * smooth +
            mouseRef.current.y * (1 - smooth);
    ...
    initializeWebGL();
    return () => cleanupFunctionRef.current && cleanupFunctionRef.current();
  }, [
    isVisible,
    raysOrigin,
    raysColor,
